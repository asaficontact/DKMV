...
- 461+ tests passing, 92%+ coverage
- All quality gates green (ruff, mypy, pytest)
- Task system fully operational: `dkmv run dev/qa/judge/docs` works
- Wrapper commands: `dkmv dev/qa/judge/docs` work via ComponentRunner
- Core infrastructure: SandboxManager, RunManager, StreamParser, DKMVConfig

**What we are building:** Project initialization that adds `.dkmv/` for config, credential discovery, component registry, and run output. Container-side rename from `.dkmv/` to `.agent/`. Make `--repo` optional when initialized.
...
def resolve_component(name_or_path: str) -> Path:
    # Will add: project_root: Path | None = None parameter
```
...
- Wrapper commands (`dev`, `qa`, `judge`, `docs`): `repo` is positional `Argument` → will become `--repo` Option
- `run_component`: `repo` is required `Option("--repo")` → will become optional
...
- `dkmv/tasks/component.py` — ComponentRunner (only Phase 4 rename of `.dkmv/` → `.agent/`)
- `dkmv/components/base.py` — BaseComponent (only Phase 4 rename)
- Existing test files — only modify for Phase 4 rename assertions
